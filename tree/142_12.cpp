...
// return the parent node of the val x node 
int findp(tnode *root,int x){
    ...
    if(root -> left) return findp(root -> left,x);
    if(root -> right) return findp(root -> right,x);
    return -1;
}

// the main method is to use the information in the stack
// return the whole ancestor of this node
void findwhole(tnode *u,int x){
    // use post order,in the stack it store the whole node which is the ancestor of this node
    cout << "find element is " << char('A' + x) << endl;
    stack<tnode *> stk;
    tnode *prev;
    ...
    while(u != nullptr || !stk.empty()){
        ...
        if(u -> right == prev || u -> right == nullptr){
            // cout << char('A' + u -> val) << endl;
            // the tree with this root node is all printed
            prev = u;
            u = nullptr;
        }else{
            stk.push(u);
...
int main(){
    ...
    */
    ...
    // find the parent of the node whose value is x
    int x = 'I' - 'A';
    // cout << char('A' + findp(root,x)) << endl;
