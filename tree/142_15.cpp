...
// use the properties of the full binary tree
// make sure that the node id is lower than n
tnode * ConstructFullTree(tnode *&u,int now,int n){
    if(now <= n){
        u = new tnode(now - 1);
        u -> left = ConstructFullTree(u -> left,now << 1,n);
        u -> right = ConstructFullTree(u -> right,(now << 1) + 1,n);
        return u;
    }else{
        return nullptr;
    }
...
int main(){
    ...
    */
    ...
    tnode *fullnode = nullptr;
    int n = 10;
    ConstructFullTree(fullnode,1,n);
    lastprint(fullnode);

    // given a full binary tree, we should get its post order by its pre order 
    // we just use the index of each node to visit it
    FullPost(1,n);
