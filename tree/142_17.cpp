...
bool judge(tnode *u1,tnode *u2){
    if(u1 == nullptr && u2 == nullptr) return true;
    if(u1 && u2) return judge(u1 -> left,u2 -> left) && judge(u1 -> right,u2 -> right);
    return false;
}
...
int main(){
    ...
    */
    ...
    tnode *root1 = nullptr,*root2 = nullptr;
    build(root1);
    lastprint(root1);
    ...
    // judge if two binary tree is simliar
    if(judge(root1,root2)) cout << "similar\n";
